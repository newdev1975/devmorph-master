...
usage() {
    log_info "Displaying workspace stop usage information"
...
if [ ! -f "$state_file" ]; then
    log_warn "Workspace '$WORKSPACE_NAME' does not have a state file"
fi
...
# Check if workspace is already stopped or not running
if [ -f "$state_file" ]; then
    ...
    if [ "$current_status" = "stopped" ] || [ "$current_status" = "created" ] || [ "$current_status" = "configured" ] || [ -z "$current_status" ]; then
        log_info "Workspace '$WORKSPACE_NAME' is already stopped or not running"
        exit 0
    fi
else
...
# Stop the workspace
log_info "Stopping workspace: $WORKSPACE_NAME"
if ! stop_workspace "$WORKSPACE_NAME"; then
    log_error "Failed to stop workspace '$WORKSPACE_NAME'"
    exit 1
fi

log_info "Workspace '$WORKSPACE_NAME' stopped successfully"
